...
namespace LUJEWebsite.PeeringApi.Payload
{
	...
	{
		public static async Task<Session> SessionPageAsync(string asn, string ixlan_id, string id, string ownerid, string afi)
		{
...
inner join peeringdb_ixlan on peeringdb_ixlan.id = peeringdb_network_ixlan.ixlan_id and peeringdb_network_ixlan.status = 'ok' and peeringdb_network_ixlan.ixlan_id = @ixlan_id
inner join peeringdb_network_ixlan as owner_peeringdb_network_ixlan on peeringdb_ixlan.id = owner_peeringdb_network_ixlan.ixlan_id and owner_peeringdb_network_ixlan.asn = @ownerasn and owner_peeringdb_network_ixlan.status = 'ok' and owner_peeringdb_network_ixlan.id = @ownerid
inner join peeringdb_ix on peeringdb_ix.id = peeringdb_ixlan.ix_id
where peeringdb_network_ixlan.asn = @asn and peeringdb_network_ixlan.id = @id;", luje_conn);
			luje_cmd.Parameters.AddWithValue("@ownerasn", Convert.ToInt32(Configuration.PortalOwnerAsn));
			luje_cmd.Parameters.AddWithValue("@asn", Convert.ToInt32(asn));
			luje_cmd.Parameters.AddWithValue("@ixlan_id", Convert.ToInt32(ixlan_id));
			luje_cmd.Parameters.AddWithValue("@id", Convert.ToInt32(id));
			luje_cmd.Parameters.AddWithValue("@ownerid", Convert.ToInt32(ownerid));
			await luje_cmd.PrepareAsync();
			luje_rdr = (MySqlDataReader)await luje_cmd.ExecuteReaderAsync();
			...
			foreach (var entry in networkList)
			{
				...
				{
					if (afi == "4" && entry.ipaddr4 != null && entry.ipaddr4 != "")
					{
						...
						session.LocalAsn = Convert.ToInt32(asn);
					...
					if (afi == "6" && entry.ipaddr6 != null && entry.ipaddr6.ToString() != "")
					{
